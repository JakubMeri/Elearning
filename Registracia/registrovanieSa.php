    ...
    header("Location: register.php?Hesla_sa_nezhoduju");
        ...
        exit();
            ...
            if($resultCheck > 0){
                ...
                $sql = "INSERT INTO uzivatelia (meno, priezvisko, email, heslo, typ_uzivatela, semester, zadanie, Skuska, spolu, odovzdane, onlineU) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                if(!mysqli_stmt_prepare($stmt, $sql)){
                    header("Location: register.php?error=sqlerror");
                    exit();
                    ...
                    $nula = 0;
                    mysqli_stmt_bind_param($stmt, "sssssssssss", $meno, $priezvisko, $email, $hashovanieHesla, $uzivatel,$nula,$nula,$nula,$nula, $nula, $nula);
                    mysqli_stmt_execute($stmt);
